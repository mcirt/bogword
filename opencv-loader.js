(function () {
  ...
  function finish(cv) {
    ...
    // This OpenCV build is thenable but is not a normal Promise.
  ...
  function registerRuntimeCallback() {
    ...
    if (!cv) {
      fail(new Error("opencv.js loaded, but window.cv was not created."));
      return;
    }
    ...
    // Known-good callback pattern from the working PPAI scanner.
  ...
  function createReadyPromise() {
    ...
    timeoutId = window.setTimeout(function () {
      fail(new Error(
        "OpenCV did not finish initializing. Fully close and reopen Safari, then try again."
      ));
    }, TIMEOUT_MS);
